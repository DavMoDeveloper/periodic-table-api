...
router.get('/', (req, res) => {
  ...
  if (q.sort) {
    const keys = q.sort.split(',').map(s => s.trim()).filter(Boolean);
  ...
  if (q.fields) {
    const fields = new Set(q.fields.split(',').map(s => s.trim()).filter(Boolean));
    pageData = pageData.map(el => Object.fromEntries(Object.entries(el).filter(([k]) => fields.has(k))));
  }
